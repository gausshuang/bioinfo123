...
def add_missing_row91():
    ...
    try:
        ...
        print("检查第91行数据:")
        row91_data = df.iloc[90]  # Excel第91行 = Python索引90
        print(f"Database name: {row91_data['Database name']}")
        print(f"URL: {row91_data['URL']}")
        print(f"Short description: {row91_data['Short description']}")
        ...
        # 检查第91行数据库是否已存在
        row91_name = str(row91_data['Database name'])
        exists = False
        for db in existing_data:
            if db['name'] == row91_name:
                exists = True
                print(f"第91行数据库 '{row91_name}' 已存在，ID: {db['id']}")
                break
        
        if not exists:
            print(f"第91行数据库 '{row91_name}' 不存在，需要添加")
            
            # 创建新的数据库条目
            new_db = {
                'id': len(existing_data) + 1,
                'name': row91_name,
                'url': str(row91_data['URL']),
                'short_description': str(row91_data['Short description']),
                'description': str(row91_data['Short description']),
                'last_update': '',
                'access': 'Free',
                'data_type': 'database',
            ...
            # 添加分类
            category = categorize_database(new_db['name'], new_db['short_description'])
            new_db['category'] = category
            new_db['category_name'] = UPDATED_CATEGORIES[category]['name']
            
            # 添加中文翻译
            new_db['short_description_zh'] = translate_description(new_db['short_description'])
            
            # 添加到数据中
            existing_data.append(new_db)
            ...
            print(f"\n成功添加第91行数据库:")
            print(f"  名称: {new_db['name']}")
            print(f"  描述: {new_db['short_description']}")
            print(f"  中文翻译: {new_db['short_description_zh']}")
            print(f"  分类: {new_db['category_name']}")
            print(f"  新的总数: {len(existing_data)}")
            
            return True
        else:
            print("第91行数据库已存在，无需添加")
            return False
            
    except Exception as e:
        print(f"添加第91行数据库时出错: {e}")
        import traceback
        traceback.print_exc()
        return False
